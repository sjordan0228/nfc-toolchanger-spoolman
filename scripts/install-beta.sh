...
write_listener() {
    ...
    # Convert Python-style toolheads list to YAML list
    # e.g. '["T0", "T1", "T2", "T3"]' → YAML list entries
    local yaml_toolheads=""
    IFS=',' read -ra th_items <<< "$(echo "$toolheads_str" | tr -d '[]"')"
    for item in "${th_items[@]}"; do
        item=$(echo "$item" | xargs)
        [[ -n "$item" ]] && yaml_toolheads+="  - \"${item}\""$'\n'
    done

    # Write config.yaml with user values
    cat > "${INSTALL_DIR}/config.yaml" <<CFGEOF
# NFC Spoolman Middleware — Configuration
# Generated by install script — edit freely, this file is never overwritten by git pull.

toolhead_mode: "${toolhead_mode}"
...
echo
echo -e "  ${BOLD}Will copy:${RESET}        ${INSTALLED_LISTENER}"
echo -e "  ${BOLD}Will write:${RESET}       ${INSTALL_DIR}/config.yaml"
...
if [[ ${#MISSING_DEPS[@]} -eq 0 ]]; then
    success "Dependencies already installed (paho-mqtt, requests, pyyaml)"
else
