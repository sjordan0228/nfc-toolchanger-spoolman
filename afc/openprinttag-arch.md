# OpenPrintTag Spool Architecture

A printer-agnostic spool data architecture for projects that support:
...
If your project supports more than AFC, then AFC should not own the data model.

Likewise, if you want tag-only operation, then Spoolman should not be required just to get color, material, temps, length, or remaining usage.
...
Your project owns the canonical spool state.
...
3. Store it in your own namespace
...
Keep the canonical data in your namespace and expose helper accessors.
...
Keep unsupported fields in your own state, such as:
...
â   âââ moonraker_db.py
â   âââ service.py
âââ adapters/
â   âââ single_tool.py
â   âââ multitool.py
â   âââ afc.py
âââ klipper/
â   âââ macros.cfg
â   âââ commands.md
âââ api/
    âââ scan_handler.py
...
```text
README-openprinttag-architecture.md
```
